...
@section('content')
    <x-breadcrumb title='Add New Menu' col="col-12 col-xl-8 col-xxl-8"
        backTo="{{ $type == 1 ? route('admin.menus.footer') : route('admin.menus.header') }}" />
    <div>
                        ...
                        <div class="box">
                            <form action="{{ route('admin.menus.store') }}" method="POST">
                                ...
                                <div class="row row-cols-1 g-3">
                                    ...
                                    <div class="col">
                                        <x-input name='icon' label="icon ( Optional )"
                                    ...
                                    <div class="col">
                                        <x-input name='url' type="url" required label="URL" />
                                    </div>
                                    <div class="col">
                                        <label class="form-label">{{ __('Open In New Tab:') }} <span
                                                class="red">*</span></label>
                                        <select class="select-input" name="is_external">
                                            <option value="0" {{ old('is_external') == '0' ? 'selected' : '' }}>
                                                {{ __('No') }}
                                            </option>
                                            <option value="1" {{ old('is_external') == '1' ? 'selected' : '' }}>
                                                {{ __('Yes') }}
                                            </option>
                                        </select>
